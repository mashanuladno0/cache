...
public:
    explicit TwoQCache(size_t cap, size_t k): capacity(cap), kin(k > 0 ? k : cap / 4) {
        if (kin >= capacity) kin = (capacity > 1) ? capacity - 1 : 0;
    }
    ...
    void print() const override {
        ...
        std::cout << "  A1:    "; for (auto& k : A1)    std::cout << k << " ";
        std::cout << "\n  Am:    "; for (auto& k : Am)    std::cout << k << " ";
        std::cout << "\n  A1out: "; for (auto& k : A1out) std::cout << k << " ";
        std::cout << "\n";
    }
};